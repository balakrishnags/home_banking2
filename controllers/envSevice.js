...
module.exports.addEnvData = async (req, res) => {
    try {
        ...
        let reqBody = ["keyName", "accessKey", "type"]
        let validateReqBody = await validateRequestBody(reqBody, req, res)
        if (validateReqBody.length > 0) {
            return res.status(400).json({
                status: 400,
                error: `Invalid properties in request body:- ${validateReqBody.join(', ')}`
            });
        }
        ...
        if (type) {
            ...
            db.query(checkSql, [type, _keyName], async (err, result) => {
                ...
                const values = [_keyName, _accessKey, type, moment().format(), moment().format(), 1];
    ...
    try {
        ...
        const getEnvDataFun = (sql, values) => {
            db.query(sql, values, (err, result) => {
                ...
                // const _result = {};
                // result.forEach(item => {
                //     const { keyName, accessKey, secretKey } = item;
                //     _result[JSON.parse(decryptData(keyName, encryptionKey))] = {
                //         "accessKey": JSON.parse(decryptData(accessKey, encryptionKey)),
                //         "secretKey": JSON.parse(decryptData(secretKey, encryptionKey))
                //     };
                // });
...
module.exports.addEnvList = async (req, res) => {
    try {
        ...
        if (name) {
            ...
            const values = [name, moment().format(), moment().format(), 1];
            addEnvDataFun(sql, values)
        } else {
...
module.exports.updateEnvList = async (req, res) => {
    try {
        ...
        if (name) {
            ...
            const values = [name, moment().format(), envName];
            db.query(sql, values, (err, result) => {
                if (err) {
                    if (err.code === "ER_DUP_ENTRY") {
                        return res.status(400).json({ status: 400, error: 'Name already exist' });
